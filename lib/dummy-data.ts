...
export function generateDummyPosts(count: number): PostType[] {
  ...
  const roles: UserRole[] = ["User", "Moderator", "Admin", "VIP"]
  const contents = [
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let j = 0; j < likesCount; j++) {
      const randomLiker = authors[Math.floor(Math.random() * authors.length)]
    ...
    for (let j = 0; j < commentsCount; j++) {
      ...
      commentTimestamp.setHours(commentTimestamp.getHours() + Math.floor(Math.random() * 12) + 1)
